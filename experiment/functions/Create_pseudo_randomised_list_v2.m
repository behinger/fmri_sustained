function my_list = Create_pseudo_randomised_list_v2(list,repeats,threshold)
% Function to generate a list of numbers from a to b, repeated c times, and
% shuffle it, ensuring no more than 2 or 3 (depending on threshold) of the
% same number in a row. For generating a pseudu-random list of trial types
...
switch threshold
    ...
    case 2
        for i = 3:length(my_list)
            % Look for 3 repeats in a row
            if my_list(i) == my_list(i-1) && my_list(i) == my_list(i-2)
                % If we find it, there is a CRIME!
                crime =1;
                % Initiate a start time for use in case we get caught in the while
                % loop
