function experiment_flicker(cfg,randomization)
%--------------------------------------------------------------------------
...
trialDistractor = setup_distractortimings(params,ntrial,params.ITI); %flicker only in ITI
...
for trialNum = 1:ntrial
    ...
    while true
        ...
        Screen('DrawTexture',cfg.win,cfg.stimTex(randomization.phase(trialNum)),[],[],params.refOrient(randomization.stimulus(trialNum)));
        draw_fixationdot(cfg,params.dotSize)
        Screen('DrawingFinished', cfg.win);
%         stimtimings(1,trialNum) = 
        
        stimOnset = Screen('Flip', cfg.win, startTime + expectedTime - cfg.halfifi)-startTime;
        
        expectedTime = expectedTime + params.stimdur.(randomization.condition{trialNum})(1);
        if strcmp('interrupt_fast',randomization.condition(trialNum))
            Screen('DrawTexture',cfg.win,cfg.stimTexRadial(randomization.phase(trialNum)),[],[],0);
        end
        draw_fixationdot(cfg,params.dotSize)

        Screen('DrawingFinished', cfg.win);
%         stimtimings(2,trialNum) =
        ...
        if strcmp('sustained',randomization.condition(trialNum))
            % just wait once till the end of the block and break out
            
            break
        end
        
        expectedTime = expectedTime + params.stimdur.(randomization.condition{trialNum})(2);
